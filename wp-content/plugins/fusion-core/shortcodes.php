...
add_shortcode('youtube', 'shortcode_youtube');
	function shortcode_youtube($atts) {
		$atts = shortcode_atts(
			...
			return '<div style="max-width:'.$atts['width'].'px;max-height:'.$atts['height'].'px;"><div class="video-shortcode"><iframe title="YouTube video player" width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://www.youtube.com/embed/' . $atts['id'] . '?wmode=transparent'.$autoplay. $atts['api_params'] .'" frameborder="0" allowfullscreen></iframe></div></div>';
	}
...
add_shortcode('vimeo', 'shortcode_vimeo');
	function shortcode_vimeo($atts) {
		...
		return '<div style="max-width:'.$atts['width'].'px;max-height:'.$atts['height'].'px;"><div class="video-shortcode"><iframe src="http'.$protocol.'://player.vimeo.com/video/' . $atts['id'] .$autoplay. $atts['api_params'] . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" frameborder="0"></iframe></div></div>';
	}
...
add_shortcode('button', 'shortcode_button');
	function shortcode_button($atts, $content = null) {
		...
		if($animation_type) {
			...
			if($animation_type != 'flash' && $animation_type != 'shake' && $animation_type != 'bounce') {
					...
					if($atts['meta_all']) {
						if($atts['layout'] == 'grid' || $atts['layout'] == 'timeline') {
							...
							if($atts['meta_tags']) {
								...
								$html .= '<span class="meta-tags">' . ob_get_clean() . '</span><span class="sep">|</span>';
							}
...
function shortcode_imageframe($atts, $content = null) {
	...
	if( $align && $align != 'none' ) {
		if( $align != 'center' ) {
			...
			$html .= "#imageframe-{$avada_imageframe_id}.imageframe{{$margin}10px;float:{$align};}";
		} else {
