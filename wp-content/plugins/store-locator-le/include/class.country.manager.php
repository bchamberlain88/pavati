...
if ( ! class_exists('SLP_Country_Manager') ) {
	 ...
	 *
	 * @package StoreLocatorPlus\Country\Manager
	...
	class SLP_Country_Manager extends SLPlus_BaseClass_Object {
		...
		/**
		 * Get country metadata by one of the attributes.
		 *
		 * @param string $value the value to look for
		 * @param string $by 'name'|'slug'|'cctld'
		 *
		 * @return SLP_Country|false the country object that matches, false if no match
		 */
		public function get_country( $value , $by='name') {
			foreach ( $this->countries as $country_slug => $country ) {
				if ( $country->$by === $value ) { return $country; }
			}
			return false;
		}
