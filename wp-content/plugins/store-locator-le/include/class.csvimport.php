    ...
    class CSVImport {
        ...
        function is_valid_csv_file( $file_meta ) {
            ...
            if (!isset($file_meta['csvfile']['name']) || empty($file_meta['csvfile']['name'])) {
                echo "<div class='updated fade'>".__('Import file name not set.','store-locator-le').'</div>';
                return false;
            }
            ...
            if ($file_meta['csvfile']['size'] <= 0)    {
                switch ( $file_meta['csvfile']['error'] ) {
                    case UPLOAD_ERR_INI_SIZE:
                        $message = __( 'Import file exceeds the upload_max_filesize in php.ini.' , 'store-locator-le' );
                        break;

                    case UPLOAD_ERR_PARTIAL:
                        $message = __( 'Import file was only partially loaded.' , 'store-locator-le' );
                        break;

                    case UPLOAD_ERR_NO_FILE:
                        $message = __( 'Import file seems to have gone missing.' , 'store-locator-le' );
                        break;

                    default:
                        $message = __('Import file is empty.','store-locator-le');
                        break;
                }
            ...
            if ($arr_file_type['type'] != 'text/csv') {
                echo "<div class='updated fade'>".
                    __('Uploaded file needs to be in CSV format.','store-locator-le')        .
                    sprintf(__('Type was %s.','store-locator-le'),$arr_file_type['type'])    .
                    '</div>';
                return false;
            }
        ...
        function move_csv_to_slpdir( $file_meta ) {
            ...
            if ( ! is_dir( SLPLUS_UPLOADDIR ) ) {
                echo "<div class='updated fade'>".
                    sprintf(
                        __('WordPress upload directory %s is missing, check directory permissions.','store-locator-le') ,
                        SLPLUS_UPLOADDIR
                    ) .
                    '</div>';
                return null;
            ...
            if (
                ...
                if ( ! rename( $file_meta['csvfile']['tmp_name'] , $new_file ) ) {
	                echo $this->slplus->helper->create_string_wp_setting_error_box(
                        __('Imported CSV file could not be renamed.','store-locator-le')      .
                        sprintf( __('Possibly out of disk space while trying to rename to %s' , 'store-locator-le') , $new_file )
	                );
	                return '';
                }
                ...
                if ( ! move_uploaded_file( $file_meta['csvfile']['tmp_name'] , $new_file ) ) {
	                echo $this->slplus->helper->create_string_wp_setting_error_box(
		                __('Uploaded CSV file could not be moved.','store-locator-le'),
		                sprintf( __('Check folder permissions for %s' , 'store-locator-le') , $new_file )
	                );
                    return '';
                }
            }
        ...
        function open_csv_file( $filename ) {
            ...
            if (($this->filehandle = fopen( $filename , "r")) === FALSE) {
                print "<div class='updated fade'>".
                    __('Could not open CSV file for processing.','store-locator-le')         . '<br/>' .
                    $new_file                               .
                    '</div>';
        ...
        function process_FileDirect( $file_meta ) {
            ...
            //
            if ($reccount > 0) {
                $this->processing_report[] = sprintf( __( '%d data lines read from the CSV file.' , 'store-locator-le') , $reccount );
            }
